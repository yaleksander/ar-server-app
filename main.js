...
app.post("/threejs", (req, res) =>
{
	...
	var contour = fs.readFileSync(__dirname + "/threejs/my-images/contour.txt", "utf8").split(" ");
...
function initialize(params)
{
	...
	vObj = new THREE.Mesh(cube, maskMat);
	fakeShadow = new THREE.Mesh(bufGeo, shadowMat);
	plane = new THREE.Mesh(planeGeo, maskMat);
	...
	rendW = parseInt(params[48]);
	rendH = parseInt(params[49]);
	preset = parseInt(params[50]);
	scene.applyMatrix4(p);
	scene.updateMatrix(true);
	scene.updateMatrixWorld(true);
	camera.updateMatrixWorld(true);

	var canvas = createCanvas(rendW, rendH);
	...
	var renderTarget = new THREE.WebGLRenderTarget(rendW, rendH,
	{
	...
	output = new Uint8Array(rendW * rendH * 4);
}
...
//
function searchWithinCap(extra, best, mask, subMax, map, ii, jj, ni, nj, ori, v0, rec, maxRec, opAlpha, ogAlpha, alpha, beta = Math.PI, theta = 0, p0 = v0.clone(), prev = 0, depth = 1, path = [])
{
	var p = [];
